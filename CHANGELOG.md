...
## [Unreleased]
- **[2026-09-26]**: fix(helpers): re-apply the issue-#1048 portable graft helpers — the re-baked machine path had re-entered via template delivery. `.claude/helpers/graft-hooks.cjs` + `graft-statusline.cjs` line 7 carried `const BAKED = "C:\Users\USER\...\nvm\v24.15.0\..."` (the pre-#1048 layout graft's session-start hook bakes in while rewriting the tracked files); on this machine the path is dead (nvm v24.15.0 gone) so graft hooks/statusline had been silently no-op'ing. Replaced both files verbatim with the L0 portable versions (dynamic resolution: project walk-up → interpreter lib dir → npm global root → untracked `graft.local.json` override, demoted last), added the `.claude/helpers/graft.local.json` gitignore rule L0 has but template delivery never carried, and created the untracked local override pointing at the bun-global `@nanonets/graft` install — statusline verified rendering again (graft · 1724 nodes / ✓ synced). Recurrence root cause documented as upstream-fix-list §9: templates/common still carries the baked helpers, so the next `upgrade-project` re-introduces the constant into every co-* project until the L0 fix is propagated.
- **[2026-09-26]**: ci(cross-platform): close the 3-OS support gaps from the 2026-09-26 platform audit. (1) Typecheck and Script Tests jobs gain an `ubuntu/windows/macos` matrix with `fail-fast: false` — Windows/macOS were previously validated only when a developer happened to run the gates locally (the 2026-07-10 ubuntu/windows matrix was dropped by the 0.6.0 template resync and PR #160's job re-introduction stayed ubuntu-only); (2) auto-merge.yml required-check matching extended to job-name prefixes (`Typecheck (windows-latest)` et al.) so the merge automation survives the matrix check-run naming — the all-checks-success gate continues to enforce every leg; (3) setup-guide prerequisites table gains the Linux row the body already assumed (vsp-linux binaries, bash utilities) and the Bash requirement is relaxed v4+ → v3.2+ after verifying the only shell scripts in the repo (.githooks/*) use no bash4-only syntax — the macOS system bash qualifies; (4) README/README_ko note the CI matrix.
- **[2026-09-26]**: chore(upgrade+drift): template 0.6.0 content sync with variant-drift resolution and AGENTS.md normalization. `upgrade-project --prune-removed` delivered dev-sync 1.20.0, validate-templates 1.46.1, verify-scripts 1.9.0, context-sections 1.8.0, scaffold-markers 1.6.1, new regenerate-agents-md.ts 1.2.0, user-guide refresh, graft helpers, and the 3 `docs/governance/agents` docs; 6 country-scoped k-* skills correctly kept out (country=none). Same-version drift resolved toward the richer template originals: `skills/performance-tuning` + `skills/post-write-chain` (owner/triggers/Context/Execution Steps restored; platform mirrors realigned via sync-skills SSOT pass) and `scripts/co-abap/vsp-publish.ts` (template hardening: commit-message control-char collapse for safe `git -m`). Scaffold-only variant configs `scripts/co-abap/atc-rulepack.json` + `tsconfig.json` delivered; stray L0-only `.codex/skills/simulate-pipeline` removed. AGENTS.md regenerated via `regenerate-agents-md.ts --source` (1,068 → 583 lines; past-upgrade COMMON block accumulation removed) with 3 stale L0-only §6 skill rows (create-variant / promote-variant / simulate-pipeline) pruned to clear the §6 path checks. Gates: audit PASS, verify-scripts 135 OK (1 upstream warning: regenerate-agents-md graft pair md not yet delivered).
- **[2026-09-26]**: docs: refresh README/README_ko and user guides to the current project state. Carries previously unreflected waves into user-facing docs: (1) agent surfaces — Codex CLI and Hermes Agent added to the architecture tier line (ADR-0077/ADR-0088; 5 skill mirrors `.claude`/`.gemini`/`.codex`/`.agents`/`.hermes`), Hermes `trusted_project_dirs` onboarding note in setup-guide; (2) quality gates rewritten to match reality — CI runs the audit + gitleaks jobs only (matrix/typecheck/test/drift jobs were dropped by the 0.6.0 template resync; audit.ts itself covers platform parity + platform lifecycle + variant checks), unit tests/typecheck documented as `/sync`-pipeline/local gates, stale MCP-drift and "3-platform drift" rows removed, test count corrected 100+ → 26; (3) deliverables 05/06 promotion reflected — user guides now describe the 6-file standard set (`04_qa_report.md` + `05_unit_test_plan.md` at Stage 4, `06_release_report.md` at Stage 5) and the new-requirement.ts scaffolding example (01+05+06); (4) agent roster table gains security-monitor and i18n-specialist rows; (5) user-guide linked from README Core Documentation; hash-sync activated for the README pair (README_ko.md `translated_from_hash` seeded) and user-guide_ko.md hash refreshed to the updated EN body.
- **[2026-09-26]**: chore(upgrade): L1 template wave delivery via upgrade-project (post-PR-#153 baseline; L0 was fast-forwarded 156 commits before the run). Delivers the ADR-0088 hermes wave: `scripts/lib/platforms.ts` 1.1.0 (fifth platform mirror `.hermes/skills`), new `.hermes/` platform directory with skill mirrors, plus updates to `scaffold-markers.ts`, `upgrade-policy.ts`, `sync-skills.ts`, `typecheck.ts`, `verify-platform-lifecycle.ts`, AGENTS.md/CLAUDE.md, `.claude/settings.json`, `ci.yml`, `workspace-schema.json`, `.env.sample`, `.gitattributes`. 7 locked + 8 merge + 13 sync + 6 tree-sync files; 6 country skills (k-*) correctly kept out of the co-abap variant; security checks PASSED. Residual L1 script drift after the run is 7 paths, all by-design (project-generated READMEs/SCRIPTS.md, variant-local `scripts/co-abap/`, L1-only delivery machinery, local `tsconfig.json`). Also reverted a local graft-updater re-bake of `.claude/helpers/graft-*.cjs` (machine-specific BAKED path — regression against the upstream portability fix).
- **[2026-09-26]**: fix(scripts): new-requirement.ts 1.1.0 — repair projectRoot resolution and extend scaffolding to the standard deliverable set. `path.resolve(scriptDir, "..")` resolved `scripts/` as the project root, so REQ folders landed under `scripts/deliverables/` and RTM insertion failed ("index.md not found") — now resolves the repo root correctly. `--help`/`-h` print usage and exit 0 instead of being parsed as the title (junk-folder root cause from the readiness check). Scaffolding now pre-creates the full standard set per requirement: `01_srs.md` + `05_unit_test_plan.md` + `06_release_report.md` (missing templates skip with a warning). Verified end-to-end: help path, real run at correct root with RTM row, then cleaned up. Design: `docs/designs/2026-09-26-new-requirement-scaffolding-design.md` (spec-registered).
- **[2026-09-26]**: docs(deliverables): standardize 05/06 as pipeline deliverables. REQ-001's `05_unit_test_plan.md` and `06_release_report.md` promoted from one-off documents to standard stage artifacts: added `deliverables/templates/05_unit_test_plan.md` + `06_release_report.md` (REQ-NNN templates), updated the Workflow Stages definition in `deliverables/index.md` (Stage 4 now carries the test plan; Stage 5 carries the release report), verified the full REQ-001 doc set (6 files) via `verify-agent-deliverables.ts`. Registration state before this change: RTM row ✔, spec registry ✔, templates ✖ → now complete. AGENTS.md §4.2 (L0-managed) left untouched — upstream alignment tracked as open item.
- **[2026-09-26]**: docs(deliverables): REQ-001 quality benchmark vs external standards + gap remediation. Benchmarked the four stage documents against the IEEE 830 / ISO/IEC/IEEE 29148 SRS structure (live-fetched) and ABAP/SAP deliverable conventions (tool checklist; ⚠️ live web search rate-limited). Verdict: covered areas above baseline (verifiable Gherkin requirements, bidirectional traceability, raw QA logs); three gaps found and closed — SRS gained §6 Constraints/Assumptions/Dependencies + §7 Glossary; new `05_unit_test_plan.md` (requirement→test traceability, IEEE 829-style expected-vs-actual); new `06_release_report.md` (Stage 5: documentation audit, `$TMP` transport status, promotion/rollback plan, handover). Review record: `docs/reports/2026-09-26-req001-benchmark-review.md`.
- **[2026-09-25]**: feat(deliverables): REQ-001 Flight Occupancy and Revenue Analysis Report — first end-to-end run of the 5-stage deliverables pipeline. Stage 1 SRS grounded in live SFLIGHT demo data (18 carriers / 14 connections / 94 flights / 27,147 bookings queried via abap MCP); Stage 2 technical design (Pattern B: local calc class + CL_SALV_TABLE); Stage 3 implemented `ZFLIGHT_OCC_REVENUE` (PROG, `$TMP`) on NPL/001 with 4 ABAP Unit test methods — OO-context `p` requires explicit LENGTH (`ty_percent` fix); Stage 4 QA gate PASSED (activation zero messages, 4/4 unit methods, ATC 0×P1, 1×P2 dispositioned accepted, coverage tool N/A in hyperfocused toolset — documented); Stage 5 RTM updated to Completed. Scratch source copy: `scratch/zflight_occ_revenue.prog.abap`.
